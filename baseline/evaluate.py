...
from model import UnifiedResNetModel  # Cambiato qui
import time
...
def main():
    ...
    # Carica modello bbox
    model = UnifiedResNetModel(head_type='bbox', pretrained=False).to(device)
    model.load_state_dict(torch.load("bbox_+_backbone_model.pth", map_location=device))
